...
export const getUserProcedures = async (planId) => {
    const url = `${api_url}/UserProcedure?$filter=planId eq ${planId}&$expand=user`;
    const response = await fetch(url, {
        method: "GET",
    });
...
export const addUserToProceudures = async (userId, planId, procedureId) => {
    const url = `${api_url}/Procedures/AddUserToProcedure`;
    var command = { userId : userId, planId: planId, procedureId: procedureId };
    const response = await fetch(url, {
        method: "POST",
        headers: {
            Accept: "application/json",
    ...
    if (!response.ok) throw new Error("Failed to create Procedure Users");
